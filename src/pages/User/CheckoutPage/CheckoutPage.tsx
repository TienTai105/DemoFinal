...
export const CheckoutPage: React.FC = () => {
  ...
  const { mutate: createOrder, isSuccess, isError, status } = useCreateOrder();
  const isLoading = status === 'pending';

  React.useEffect(() => {
    if (isSuccess) {
      // When mutation reports success, finalize UI updates here to ensure overlay shows
      reset();
      setIsSubmitting(false);
      setShowSuccess(true);
      getClearCart();
    }
  }, [isSuccess, reset, getClearCart]);
        ...
        <Col lg={4}>
          ...
          <div className="order-summary">
            ...
            <Button
                  ...
                  className="mt-4"
                >
                  {isSubmitting || isLoading ? 'Processing...' : 'Place Order'} <ChevronRight size={20} />
                </Button>
